...
{
    public PixiesFileEntry[] entries;
    ...
    public class PixiesModule
{
    ...
    public PixiesModule()
    {
        ...
        this.colorPalette = new Color32[256];
    ...
    public PixiesFile LoadPixies(TwtFileEntry entry)
    {
        ...
        while (pixReader.BaseStream.Position < pixReader.BaseStream.Length)
        {
            ...
            switch (blockType)
            {
                ...
                case PixBlockType.ImageData:
                    ...
                    Texture2D bmp = new Texture2D(currentWidth, currentHeight);

                    for (int y = 0; y < currentHeight; y++)
                    {
                        for (int x = 0; x < currentRowSize; x++)
                        {
                            Color32 c = ReadColor(pixReader, currentSubType);

                            if (x < currentWidth)
                            {
                                bmp.SetPixel(x, currentHeight - 1 - y, c);
                            }
                        }
                    }
    ...
    private Color32 ReadColor(BinaryReader pixReader, PixSubType subType)
    {
        switch (subType)
        {
            ...
            case PixSubType.Bit24:
                return new Color32(pixReader.ReadByte(), pixReader.ReadByte(), pixReader.ReadByte(), 0);
            case PixSubType.ColorPalette:
                byte a = pixReader.ReadByte();
                byte r = pixReader.ReadByte();
                byte g = pixReader.ReadByte();
                byte b = pixReader.ReadByte();
